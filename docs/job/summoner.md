...
召唤的AOE技能是<Action name="迸发" />，另外可以通过<Action name="能量抽取" />获得以太，消耗以太可以使用<Action name="痛苦核爆" />，DOT技能对应的AOE技能则是<Action name="灾祸" job="秘术师" />（俗称传染）。

召唤在面对多个敌人的时候，首先上双毒（或者直接使用三灾），然后使用<Action name="灾祸" job="秘术师" />把DOT扩散给所有敌人，接下来使用<Action name="迸发" />填充，用<Action name="能量抽取" />获得<Status :id="304" name="以太超流" />，然后使用<Action name="痛苦核爆" />。
