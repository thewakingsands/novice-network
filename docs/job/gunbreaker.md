...
枪刃的无敌是<Action name="超火流星" />，<Status :id="1836" name="超火流星" />自身的buff和骑士的<Status :id="1302" name="神圣领域" />效果本身是一样的，但枪刃在使用<Action name="超火流星" />时，会瞬间扣到1血，给很多治疗巨大的精神冲击。同时由于游戏系统判定时间差原因，骑士若在伤害判定之后才开启神圣领域，并不会产生无敌效果；但枪刃的超火流星会在按下技能的瞬间锁定1血，哪怕是在伤害判定后才开启超火流星，也可以保证至少有1血存活。通常不建议枪刃过晚开超火流星，也不建议在面对白魔以外的治疗开超火流星——先<Action name="天赐祝福">天赐</Action>后超火流星，倒计时10s后倒T的惨剧，古今中外数不胜数。
