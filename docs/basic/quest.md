...
* 主线20级：<quest name="我的专属陆行鸟" type="plus" />，完成20级主线任务<quest name="前行之路" type="main"/>并加入大国防联军后接取，完成任务可获得坐骑专属陆行鸟（在获得专属陆行鸟之前，商城购买的坐骑也无法使用）。
...
理符任务现在并不是重要的任务系统，但是由于它以前占有很大的游戏比重，因此稍显复杂。在完成出生主城的<quest name="挑战理符" type="plus" />任务之后，各地会出现==XX理符开放任务==，完成对应地区的理服开放任务之后，就可以接取当地的理符任务了。
