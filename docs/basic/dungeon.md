...
在菜单 > 任务情报 > 任务搜索器（默认快捷键`U`）可以打开任务搜索器，勾选自己想要参加的副本并点击下方的==参加== （最多可同时勾选5个指定副本），就可以由系统自动匹配其他玩家。

> * [副本攻略](/duty/)：如果能在参加副本之前预习一下攻略，是体面的行为。本站收录的副本攻略均无剧透，可放心阅读。
> * [挑战副本](/topic/battle.md)：如果想要挑战高难度副本，可以阅读这里的介绍了解高难度副本的分类、挑战顺序等建议。
...
在输入框下方点击齿轮图标，然后点击==人名显示设置==，可以调整发言者的相关信息设置：

* 通用：显示/隐藏其他服务器玩家的服务器名
* 小队/团队：当玩家在小队/团队频道发言时，职业/职能的显示规则。
  * 可以在下方预览窗口中查看显示效果，推荐显示职业图标+整个人名改变颜色。

修改人名显示设置后，就可以更轻松地区分发言者的职业了！
...
;;;.guide .cols2
;;;.guide .col
<img src="./battle.assets/16826718363712.jpg" width="350px" />
;;;
...
;;;
;;;
> 进入副本以及最优队员推荐窗口默认显示的是[玩家肖像](/topic/plate.md)，可以在==菜单 > 角色 > 肖像列表==中进行编辑和修改。
>
> 如果不喜欢这个窗口，可以在==角色设置 > 界面设置 > 全体 > 任务用肖像==中进行设置。
...
如果没多少时间打随机，推荐优先打==练级==和==讨伐==，==团队任务==也可以选择，==主线任务==耗时较长（有不可跳过的任务动画），但奖励也较多，如果有时间推荐打。
...
- 限制“随机任务：练级”的随机目标：队长打开后，当组满4个人参加随机练级任务时，副本等级不会比队伍内等级最低的角色等级低超过8级。
...
;;;.guide .cols2
;;;.guide .col
![](./battle.assets/Trust.jpg)
;;;

;;;.guide .col .grow

71级之后的副本，可以使用亲信战友系统进行反复攻略。

在==菜单 > 任务情报 > 亲信战友==中可以打开亲信战友菜单。在亲信战友中，使用名为“幻体”的主线剧情中的伙伴NPC，他们具有初始等级，通关副本后，玩家和幻体都可以获得经验值。**和幻体挑战副本时，通关所获得的奖励会少于与其他玩家通关副本时获得的奖励。**

幻体80级后会开放==投影==功能，解锁幻体NPC的不同外观。同时为幻体升级还可以获得成就、称号。

;;;
;;;

> 没有等级限制，可以随时挑战副本的系统为[剧情辅助器](/basic/battle.md#副本迷宫)。
