...
;;;.guide .col .grow
点击登录器右上角齿轮设置，可开启DX11支持，**建议务必打开**。原则上现在FF14已经不支持DX9了，如果继续使用DX9的话很容导致报错及游戏崩溃。

游戏更新时有可能会重置DX设置，每次版本更新后如果发现游戏卡顿，可先检查一下DX设置是否被重置。
;;;
;;;

菜单 > 系统 > 角色设置/系统设置中有很多可以手工设置的内容，下面仅摘要部分会对游戏体验有较大改善的设置进行介绍：
...
| 常见调整方式 | 将`A` `D`键改为左右平移;<br>在需要移动施法时，按住<i class="xiv mouse-left-right-button"></i>轻甩鼠标保持面向。 | 需要左右平移时，按住<i class="xiv mouse-left-button"></i>或<i class="xiv mouse-right-button"></i>的同时按`A` `D`；<br>需要保持正面倒退步行时，需要同时按住`Q` `E`和`S` ；<br>需要观测BOSS同时移动时，根据自己与BOSS的夹角选择移动方向。 |
...
* 不显示未勾选的格子：可以选中保持美观，隐藏方便用来做特殊的布局。
...
如果画面卡顿比较严重，难以流畅运行的话，可以从以下设置开始降低：
* 实时反射：标准品质或关闭
* 抗锯齿：关闭
* 影子：细节层次关闭，子选项全部调到最低
* 摇晃表现：除自己全关闭
...
如果关闭特效后仍有卡顿，可尝试使用[MAGPIE](https://github.com/Blinue/Magpie)放大游戏画面，该工具可以将标清窗口画面放大到4K，也可以将小窗口放大到标清。
...
有3种UI配色主题可选，默认为深色，另两种为浅色和经典（蓝色）。
